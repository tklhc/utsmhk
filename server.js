...
// [FIX #9] Path validation + companyId numeric check + SSRF protection (S7044)
const SAFE_PATH_SEGMENT = /^[a-zA-Z0-9_\-]+$/;

function buildSafeParasutPath(companyId, rawPath) {
  // 1. URL-decode → encoded bypass'ları yakala (%2e%2e → ..)
  let decoded;
  try {
    decoded = decodeURIComponent(rawPath);
  } catch {
    return { error: "Geçersiz URL encoding" };
  }

  // 2. Decode sonrası path traversal kontrolü
  if (/\.\./.test(decoded)) {
    return { error: "Geçersiz path: traversal tespit edildi" };
  }

  // 3. Segmentlere ayır ve doğrula
  const segments = decoded.split("/").filter(Boolean);
  if (segments.length === 0 || segments.length > 5) {
    return { error: "Geçersiz path yapısı" };
  }

  // 4. İlk segment = resource → whitelist kontrolü
  if (!PARASUT_ALLOWED.includes(segments[0])) {
    return { error: "İzin verilmeyen kaynak: " + segments[0] };
  }

  // 5. Her segment'te sadece güvenli karakterler
  for (const seg of segments) {
    if (!SAFE_PATH_SEGMENT.test(seg)) {
      return { error: "Path segmentinde geçersiz karakter" };
    }
  }

  // 6. Güvenli path oluştur — her segment ayrıca encode edilir
  return {
    path: "/v4/" + encodeURIComponent(companyId) + "/" +
...
app.all("/api/parasut/v4/:companyId/*", async (req, res) => {
  ...
  try {
    ...
    const cfg = readParasutConfig();
    if (cfg.companyId && cid !== cfg.companyId) return res.status(403).json({ error: "Yetkisiz şirket erişimi" });

    // SSRF-safe path oluşturma
    const pathResult = buildSafeParasutPath(cid, req.params[0]);
    ...
    // Query string — güvenli URL API ile
    const parsedUrl = new URL(req.url, `http://${req.headers.host || "localhost"}`);
    const safeQuery = parsedUrl.search || "";

    const result = await parasutProxy(
      req.method,
      pathResult.path + safeQuery,
